...
export class ScholarTool {
  ...
  async searchPapers(topic: string, limit: number = 5): Promise<Paper[]> {
    ...
    try {
      ...
      const results = await Promise.allSettled([
        this.searchArxiv(topic, perSourceLimit),
        this.searchOpenAlex(topic, perSourceLimit),
        this.searchSemanticScholar(topic, perSourceLimit)
      ]);
  ...
  private async searchOpenAlex(topic: string, limit: number): Promise<Paper[]> {
  ...
  private async searchSemanticScholar(topic: string, limit: number): Promise<Paper[]> {
