...
experiments_dir = project_dir.parent / "fin-agent-group" / "experiments"
...
亮点：[列出2-3个文献综述亮点]"""
    ...
    def _evaluate_with_llm(self, paper_content, dimension):
        """使用LLM评估单个维度"""
        if not self.llm_client:
            # LLM不可用，使用模拟评估
            return self._heuristic_evaluate(paper_content, dimension)
        
        try:
            prompt = self._get_evaluation_prompt(dimension, paper_content)
            system_prompt = "你是一个专业、严格但公正的学术审稿人。请根据提供的评估标准，对论文进行客观、详细的评估。"
            
            print(f"   🤖 使用LLM评估: {dimension}...")
    ...
    def _heuristic_evaluate(self, paper_content, dimension):
        ...
        # 基础评分 2.0-4.0
        base_score = 2.0 + min(2.0, count * 0.15)
    ...
    def evaluate_paper(self, paper_content, paper_title="Untitled Paper"):
        ...
        for dim in self.evaluation_dimensions:
            ...
            # 使用LLM评估
            eval_result = self._evaluate_with_llm(paper_content, dim['name'])
            ...
            if eval_result['reason']:
                print(f"   理由: {eval_result['reason'][:100]}...")
    ...
    def compare_papers(self, paper_paths):
        """对比多篇论文"""
        print(f"\n{'='*80}")
        print(f"📊 LLM论文对比评估")
        print(f"{'='*80}")
        ...
        for paper_path in paper_paths:
            ...
            result = self.evaluate_paper(content, title)
        ...
        if all_results:
            ...
            # 找出最高分
            best_result = max(all_results, key=lambda x: x['total_score'])
            best_index = all_results.index(best_result) + 1
            print(f"\n🏆 最高评分: 论文 {best_index} ({best_result['title']}) - {best_result['total_score']:.2f}/5.0")
            print(f"{'='*80}")
...
def main():
    ...
    paper_paths = [
        project_dir / 'optimized-paper-2026-03-03.md',
        project_dir / 'further-optimized-paper-2026-03-03.md',
        project_dir / 'ultimate-optimized-paper-2026-03-03.md'
    ]
    ...
    for path in paper_paths:
        ...
        print(f"论文 {len(valid_paths)+1} 路径: {path}")
        print(f"论文 {len(valid_paths)+1} 存在: {exists}")
        if exists:
            valid_paths.append(str(path))
    
    if valid_paths:
        # 对比论文
        evaluator.compare_papers(valid_paths)
    else:
        print("⚠️  没有找到有效的论文文件")
