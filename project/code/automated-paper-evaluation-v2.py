...
class PaperEvaluator:
    ...
    def _heuristic_evaluate(self, content):
        ...
        # 维度1: 原创性与新颖性
        novelty_keywords = ['novel', 'new', 'innovative', 'first', 'unprecedented', 'breakthrough', 'paradigm', 'fundamentally', 'reimagines']
        novelty_count = sum(1 for kw in novelty_keywords if kw in content_lower)
        scores[0] = min(5.0, 2.0 + novelty_count * 0.25)
        
        # 维度2: 技术质量与严谨性
        rigor_keywords = ['method', 'experiment', 'result', 'analysis', 'validation', 'hypothesis', 'statistical', 'significance', 'confidence', 'ablation', 'power analysis', 'replication']
        rigor_count = sum(1 for kw in rigor_keywords if kw in content_lower)
        scores[1] = min(5.0, 2.0 + rigor_count * 0.18)
        
        # 维度3: 清晰度与表达
        structure_keywords = ['abstract', 'introduction', 'method', 'result', 'discussion', 'conclusion', 'reference', 'table', 'figure']
        structure_count = sum(1 for kw in structure_keywords if kw in content_lower)
        scores[2] = min(5.0, 2.0 + structure_count * 0.28)
        
        # 维度4: 相关性与影响力
        impact_keywords = ['important', 'significant', 'impact', 'contribution', 'benefit', 'value', 'implications', 'broader', 'real-world']
        impact_count = sum(1 for kw in impact_keywords if kw in content_lower)
        scores[3] = min(5.0, 2.0 + impact_count * 0.28)
        
        # 维度5: 文献引用与上下文
        citation_keywords = ['cite', 'reference', 'bibliography', 'et al', '202', '201', '200', '199', 'arxiv', 'doi', 'proceedings']
        citation_count = sum(1 for kw in citation_keywords if kw in content_lower)
        scores[4] = min(5.0, 2.0 + citation_count * 0.22)
    ...
    def compare_three_papers(self, paper1_path, paper2_path, paper3_path):
        """对比三篇论文"""
        print(f"\n{'='*80}")
        print(f"📊 三篇论文对比评估")
        print(f"{'='*80}")
        ...
        # 读取论文3
        with open(paper3_path, 'r', encoding='utf-8') as f:
            content3 = f.read()
        title3 = os.path.basename(paper3_path)
        
        # 评估三篇论文
        result1 = self.evaluate_paper(content1, title1)
        result2 = self.evaluate_paper(content2, title2)
        result3 = self.evaluate_paper(content3, title3)
        ...
        for label, title, result in results:
            print(f"\n{label}: {title}")
            print(f"  综合评分: {result['total_score']:.2f}/5.0")
            print(f"  等级: {result['grade']}")
        
        # 找出最高分
        best_result = max(results, key=lambda x: x[2]['total_score'])
        print(f"\n🏆 最高评分: {best_result[0]} ({best_result[1]}) - {best_result[2]['total_score']:.2f}/5.0")
...
def main():
    ...
    paper3_path = os.path.join(project_dir, 'ultimate-optimized-paper-2026-03-03.md')
    
    print(f"论文 1 路径: {paper1_path}")
    print(f"论文 2 路径: {paper2_path}")
    print(f"论文 3 路径: {paper3_path}")
    print(f"论文 1 存在: {os.path.exists(paper1_path)}")
    print(f"论文 2 存在: {os.path.exists(paper2_path)}")
    print(f"论文 3 存在: {os.path.exists(paper3_path)}")
    
    # 对比三篇论文
    evaluator.compare_three_papers(paper1_path, paper2_path, paper3_path)
